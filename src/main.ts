...
interface Options {
  getXValues: (data: TimeSeriesDatum) => string; //,
  ...
  xType?: typeof d3.scaleBand; // = d3.scaleUtc, // the x-scale type
  xRange?: [number, number]; // = [marginLeft, width - marginRight], // [left, right]
  yType?: typeof d3.scaleLog | typeof d3.scaleLinear; //
...
function LineChartLog(
  ...
  // Compute values.
  const xAxisValues: string[] = d3.map(data, getXValues);
  ...
  const xScale = xType(xAxisValues, xRange);
  const yScale = yType([1, d3.max(yAxisValues)!], yRange);

  const renderXAxis: d3.Axis<string> = d3
    .axisBottom(xScale)
    .ticks(60)
    .tickSizeOuter(0);
...
keysOf(totalEmissions).map((foodType) => {
  keysOf(totalEmissions[foodType]).map((year) => {
    data.push({
      year: year,
      value: Math.round(
        Number(totalEmissions[foodType][year]) / 1000_000_000_000
      ),
      food: foodType,
    });
  });
...
const chart2 = LineChartLog(data, {
  getXValues: (d) => d.year,
  getYValues: (d) => d.value,
  getZValues: (d) => d.food,
  yLabel: "Btonnes CO2",
...
if (chart) {
  ...
  document.getElementById("logChart")?.append(chart2);
}
